...
public class Weapons {
    ...
    public WeaponName getWeaponAtPosition(int pos) {
        return weaponsList.get(pos);
    }
    ...
    public void removeWeapon(WeaponName weaponName) {
        weaponsList.remove(weaponName);
    }
    ...
    public void setUpName(WeaponName weaponName) {
        this.Name = weaponName.name();
    }

    public void setUpWeapons(WeaponName weaponName) {
        switch (weaponName) {
            case KNIFE -> {
                setUpDamage(8);
                setUpName(WeaponName.KNIFE);
                break;
            }
            case GANDALFSTAFF -> {
                setUpDamage(18);
                setUpName(WeaponName.GANDALFSTAFF);
                break;
            }
            case ELRONDSRING -> {
                setUpDamage(8);
                setUpName(WeaponName.ELRONDSRING);
                break;
            }
            case GLAMDRING -> {
                setUpDamage(8);
                setUpName(WeaponName.GLAMDRING);
                break;
            }
            case ORCRIST -> {
                setUpDamage(8);
                setUpName(WeaponName.ORCRIST);
                break;
            }
            case ONERING -> {
                setUpDamage(8);
                setUpName(WeaponName.ONERING);
                break;
            }
            case NARSIL -> {
                setUpDamage(8);
                setUpName(WeaponName.NARSIL);
                break;
            }
            case ELFBOW -> {
                setUpDamage(8);
                setUpName(WeaponName.ELFBOW);
                break;
            }
            case STING -> {
                setUpDamage(8);
                setUpName(WeaponName.STING);
                break;
            }
            case AXE -> {
                setUpDamage(8);
                setUpName(WeaponName.AXE);
                break;
            }
    ...
    public String getWeaponDescription(WeaponName weaponName) {
        ...
        switch (weaponName) {
            ...
            case ORCRIST -> tempWeaponDesc = " Made by the Elves of Gondolin, Orcrist was forged along with its sister blade, Glamdring. Like Glamdring and Sting, it would glow blue whenever enemies were near.\n" +
            ...
            case ELRONDSRING -> tempWeaponDesc = "Vilya was the strongest of the three Elven rings. Its exact powers are unknown, but it is believed that it gave its bearer healing powers and control over minor elements, such as water. Elrond’s ability to summon a torrent of water to chase off the Ringwraiths is thought to be in part powered by Vilya.  \n" +
                    "\nHistory: Just before the death of Gil-galad, the High King of the Ñoldor in Middle-Earth, the ring was given to Elrond.";
            case GANDALFSTAFF -> tempWeaponDesc = "Gandalf had two staffs. The first was made of gnarled wood and cradled a jewel at the top that would light up. His second staff was white with a slightly more geometric design.\n" +
